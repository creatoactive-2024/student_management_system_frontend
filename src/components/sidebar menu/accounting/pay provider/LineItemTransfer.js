...
import { Table, Space, Modal, message, Select, Form, DatePicker } from "antd";
...
const LineItemTransfer = () => {
  ...
  const onFinish = async () => {
    ...
    const collectionName = "lineitemsaccommodations";
    const data = {
  ...
  const columns = [
    ...
    {
      title: "Comment for additional line",
      dataIndex: "",
      key: "",
    },
  ];
  ...
  const handleEditButtonClick = async () => {
    try {
      ...
      const response = await axios.post(`${baseURL}/getdata`, {
        collectionName: "lineitemsaccommodations",
        id: selectedRecordId,
      });
        ...
        >
          <Space>
            <Space>
              ...
              <Select style={{ width: 155 }} placeholder="--Groups--">
                <Option value="option1">option1</Option>
                <Option value="option">option</Option>
              </Select>
            </Space>{" "}
...
export default LineItemTransfer;
