...
const EmailLayout2 = () => {
  ...
  const onFinish = async () => {
    ...
    const collectionName = "emailcooplayouts";
    const data = {
  ...
  const handleEditButtonClick = async () => {
    try {
      ...
      const response = await axios.post(`${baseURL}/getdata`, {
        collectionName: "emailcooplayouts",
        id: selectedRecordId,
      });
  ...
  return (
    <>
      <Breadcrumb>
        ...
        <Breadcrumb.Item>Email Layout 2 (co-op)</Breadcrumb.Item>
      </Breadcrumb>
      <hr />
      <div>
        <div
