...
import { Table, Form, Space, Modal, message, Select, Breadcrumb } from "antd";
...
const Periods = () => {
  ...
  const fetchData = async () => {
    ...
    await fetchDataCommon("periods", setData, setSelectedRowKeys, setLoading);
  };
  ...
  const columns = [
    ...
    {
      title: "Teacher",
      dataIndex: "period_teacher_payments",
      key: "period_teacher_payments",
    },
    {
      title: "Transfer",
      dataIndex: "period_transfer_payments",
      key: "period_transfer_payments",
    },
    {
      title: "Accommodation",
      dataIndex: "period_accomodation_payments",
      key: "period_accomodation_payments",
    },
    {
      title: "Created by",
      dataIndex: "",
  ...
  const transformedData = data
    ? data.map((entry) => ({
        ...
        from: entry.from ? new Date(entry.from).toLocaleDateString() : null,
        end: entry.end ? new Date(entry.end).toLocaleDateString() : null,

        activities: entry.activities ? "Yes" : "No",

        insurance: entry.insurance ? "Yes" : "No",
        period_transfer_payments: entry.period_transfer_payments ? "Yes" : "No",
        period_teacher_payments: entry.period_teacher_payments ? "Yes" : "No",
        period_accomodation_payments: entry.period_accomodation_payments
          ? "Yes"
          : "No",
  ...
  return (
    <>
      <Breadcrumb>
        ...
        <Breadcrumb.Item>Periods</Breadcrumb.Item>
      </Breadcrumb>
      <hr />
      <div>
        <div
