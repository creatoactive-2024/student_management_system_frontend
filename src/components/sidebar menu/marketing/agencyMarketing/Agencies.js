...
const Agencies = () => {
  ...
  //---------------------table fetch data function----------------------------------------
  const fetchData = async () => {
    // Call the common fetch API with the selected record IDs
    await fetchDataCommon("agents", setData, setSelectedRowKeys, setLoading);
  };
  ...
  const columns = [
    ...
    {
      title: "Group",
      dataIndex: "agent_group_name",
      key: "agent_group_name",
    },
    {
      title: "Active",
      dataIndex: "active",
  ...
  const transformedData = data
    ? data.map((entry) => ({
        ...
        address_addon: entry.address_addon || null,
        pincode: entry.pincode || null,
        category_name: entry.category_name || null,
        agent_group_name: entry.agent_group_name || null,
